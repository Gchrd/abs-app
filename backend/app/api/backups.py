...
def recalculate_hashes(current_user=Depends(require_admin), db: Session = Depends(get_db)):
    """
    Admin utility: Re-read each backup file from disk and recompute hash using
    the current sanitize_config logic (with proper newline normalization).

    This fixes the 'Changed but no diff' false positives caused by \\r\\n vs \\n
    inconsistencies in hashes computed before the normalization fix was applied.
    """
    from ..utils.config_sanitizer import sanitize_config
    ...
    for b in backups:
        ...
        try:
            ...
            errors.append(f"Backup #{b.id}: {e}")

    db.commit()

    audit_event(
        user=current_user.username,
        action="backup_recalculate_hashes",
        target="all_backups",
        result=f"success (updated={updated}, skipped={skipped}, errors={len(errors)})"
    )

    return {
        "message": "Hash recalculation complete.",
        "updated": updated,
        "skipped": skipped,
        "errors": errors,
    }
